# Motos America Service Advisor Academy — Web Training Site

A mobile-friendly web version of the Service Advisor Academy manual: read all 21
modules, take instantly-scored quizzes, and unlock two full Part exams. Each
module review requires a perfect score to pass — a wrong answer sends the
trainee back to re-read the module rather than showing which answers were
right or wrong, so people have to actually revisit the material rather than
guess their way through. Managers get a report showing who's registered,
who's passed each module, how many tries it took them, and each Part exam
score.
...
- **This should be a separate Supabase project from the Sales Academy site** — keeps Service trainee data and Sales trainee data cleanly apart, with no risk of one academy's schema changes affecting the other.
...
**Before running it:** the store list matches the Sales Academy site's five stores, including **MA Corporate** (a manager-only "store" for corporate staff, not a physical dealership):
```sql
...
Roles for this academy are **Service Advisor** and **Manager** (an `admin` role also exists in the database for anyone who needs full access without being tied to a specific store's manager view — that role isn't in the login dropdown; create it directly in Supabase's Table Editor if needed).
...
- Save. GitHub will give you a URL like `https://motosamerica.github.io/service-academy/` within a minute or two
...
- **You'll also need the logo image** (`MA_logo_white_header.png`) in this repo, same file used on the Sales Academy site — grab it from that repo and add it here, since it's the shared corporate logo, not Sales-specific.
